...
class RobustGNN(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch):
        ...
        x = F.leaky_relu(self.conv3(x, edge_index), negative_slope=0.01)
        x = F.leaky_relu(self.conv4(x, edge_index), negative_slope=0.01)
        x = F.leaky_relu(self.conv5(x, edge_index), negative_slope=0.01)
        x = global_add_pool(x, batch)  # ONLY CHANGE
        return self.lin(x)
...
print("Starting training Version LeakyReLU Sum...")
