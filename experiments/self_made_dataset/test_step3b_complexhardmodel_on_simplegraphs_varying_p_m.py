...
from torch_geometric.nn import GCNConv, global_max_pool # Changed to MAX pool
...
class RobustGNN(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch):
        ...
        # MUST MATCH TRAINING: Using max_pool
...
def run_test():
    ...
    print(f"--- TESTING ROBUST MODEL (LARGE GRAPHS: 100 NODES) ---")
    print(f"Using Seed: {GLOBAL_SEED}")
    
    for label, sweep_type, vals in [("ER Density Sweep", "ER", p_values), ("BA Connectivity Sweep", "BA", m_values)]:
        print(f"\nResults for {label}:")
        print("-" * 50)
        for val in vals:
            trial_accs = []
            for _ in range(NUM_TRIALS):
                # Testing on 100 nodes to check scale-invariance
            ...
            print(f"{sweep_type} {val:<5} | Mean Accuracy: {np.mean(trial_accs):.2f}% | Std: {np.std(trial_accs):.2f}%")
